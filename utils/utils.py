...
def compute_result(dataloader, net):
    ...
    for img, cls in tqdm(dataloader):
        clses.append(cls)
        bs.append((net(img.cuda(),is_train=False)).data.cpu())
...
def evalModel(test_loader, dataset_loader, net, Best_mAP, bit, config, epoch, f):
    ...
    if mAP > Best_mAP:
        ...
        if "logs_path" in config:
            ...
            elif "nuswide_21" in config["dataset"]:
                P, R = pr_curve(trn_binary.numpy(), tst_binary.numpy(), trn_label.numpy(), tst_label.numpy(), draw_range=nuswide_21_d_range)
