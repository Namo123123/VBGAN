...
class Generator(torch.nn.Module):
    def __init__(self, channels, bits):
        super().__init__()
        ...
        self.main_module = nn.Sequential(
            # Z latent vector 100
            nn.ConvTranspose2d(in_channels=bits, out_channels=1024, kernel_size=4, stride=4, padding=0),
            nn.BatchNorm2d(num_features=1024),
            nn.ReLU(True),

            # State (1024x4x4)
            nn.ConvTranspose2d(in_channels=1024, out_channels=512, kernel_size=4, stride=4, padding=1),
...
class Discriminator(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.output = nn.Sequential(
            nn.Conv2d(in_channels=512, out_channels=1, kernel_size=4, stride=4, padding=1),
            # Output 1
            nn.Sigmoid())
